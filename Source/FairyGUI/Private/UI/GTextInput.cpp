...
void UGTextInput::SetOnlyNumeric(bool bInOnlyNumeric)
{
	bOnlyNumeric = bInOnlyNumeric;
}

void UGTextInput::SetRegexPattern(FString InRegexPattern)
{
	bHasRegexPattern = !UKismetStringLibrary::Trim(InRegexPattern).IsEmpty();

	if (bHasRegexPattern)
		RegexPattern = FRegexPattern(InRegexPattern);
}
...
int32 UGTextInput::GetMaxLength() const
{
	return MaxLength;
}
